...
//! `tokio::net` (ported from `unidpp-resolver`'s `httpc`, which is the
//! house pattern for integration tests — no reqwest/hyper-client/TLS
//! dependency). `http://` only; integration servers bind loopback.
