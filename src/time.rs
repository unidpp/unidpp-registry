...
//!
//! Ported from `unidpp-resolver`'s `time` (itself ported from
//! `unidpp-core`'s `unidpp_model::time` — same doctrine): hand-rolled
//! on purpose — no external time dependency, exact integer arithmetic
//! (seconds since the UNIX epoch; sub-second precision is not needed
//! at the registry), strict parsing, canonical display with the
//! trailing `Z`. All registry stamps are UTC; local time is a
